...
| [0058](0058-sd-mirror-concurrent-profiling.md) | SD + mirror concurrent-load profiling harness — SD (0055) and the mirror (0050) had never run in the same firmware image before this; adds opt-in per-stage timing (capture/encode/publish) and a combined bench sketch (inline vs. dedicated-task SD I/O) to actually measure the reported concurrent-performance symptom instead of assuming a cause | Proposed — written, not yet built or flashed (no toolchain/hardware in-session) |
